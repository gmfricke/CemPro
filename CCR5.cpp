...
CCR5::CCR5( World* world, int init_x, int init_y, int init_z ) : Protein( world, init_x, init_y, init_z )
{
}
...
void CCR5::move()
{
  ...
  // bind two CD4s.
	...
	if ( bound_cd4 ) 
	  {
	    cout << "CCR5: found existing cd4 bond" << endl;
	    cd4_bound = 1;
	  }

	if ( bound_ccr5 ) 
	  {
	    cout << "CCR5: found existing ccr5 bond" << endl;
	    ccr5_bound = 1;
	  }
	...
	if ( delta_energy > 0 ) // i.e. delta_energy is positive so move with some probability - if negative we always move
	{
		...
		{
		  cout << "CCR5: not moving (delta energy = " << delta_energy << ") " << probability_of_move << " < " << r << endl;
			return;
		}
		else
		{
		  cout << "CCR5: moving: (delta energy = " << delta_energy << ") " << probability_of_move << " > "<< r << endl;
		}
	}
...
void CCR5::draw()
{
  ...
  if (bound_cd4) glColor3f(1.0, 0.0, 1.0);
  else glColor3f(0.5, 0.0, 0.75);
...
void CCR5:: bindCD4( CD4* partner )
{
  if (bound_cd4) bound_cd4->unbindCCR5();
  bound_cd4 = partner;
  if (bound_cd4->getBoundCCR5() != this) bound_cd4->bindCCR5( this );
}
...
CD4* CCR5::getBoundCD4()
{
  return bound_cd4;
}

CCR5* CCR5::getBoundCCR5()
{
  return bound_ccr5;
}

bool CCR5::isUnboundToCD4()
{
  return bound_cd4 == NULL;
}

bool CCR5::isUnboundToCCR5()
{
  return bound_ccr5 == NULL;
}

void CCR5:: bindCCR5( CCR5* partner )
{
  if (bound_ccr5) bound_ccr5->unbindCCR5();
  bound_ccr5 = partner;
  if (bound_ccr5->getBoundCCR5() != this) bound_ccr5->bindCCR5( this );
}

void CCR5::unbindCCR5()
{ 
...
CD4* CCR5::getAnAdjacentUnboundCD4( int x, int y, int z)
{
...
CCR5* CCR5::getAnAdjacentUnboundCCR5( int x, int y, int z )
{
  Site this_site = the_world->sites[x][y][z];
  int ccr5_count = 0;
  Site* site = NULL;
